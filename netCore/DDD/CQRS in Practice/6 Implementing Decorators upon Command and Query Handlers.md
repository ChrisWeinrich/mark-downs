...
Database retry => Handle has to be runned in a loop

## Database Retry Decorator

Add Transient with Factory Overload


```c#
services.AddTransient<ICommandHandler<EditPersonalInfoCommand>>(x => new DatabaseRetryDecorator<EditPersonalInfoCommand>(new EditPersonalInfoCommand(),config));
...
modifies Behavior of existing without changing public interface
...
Use Decorators over middleware
...
**don't reuse command handlers, don`t raise them inside other handlers**
